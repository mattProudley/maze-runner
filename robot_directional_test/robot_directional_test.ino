...
void calculateDirection () {
  ...
  else if (lDistance > threshold) { //if robot left is too close to object
    newDir = 'R';                    //turn right
    }

  else if (rDistance > threshold) { //if robot right is too close to object
    newDir = 'L';                    //turn left
    }
...
void setDirection() {
  ...
  else if (newDir == 'L') { //left
    digitalWrite(motorLF, LOW);
    digitalWrite(motorLB, HIGH); //left wheel backward
    digitalWrite(motorRF, HIGH);  //right wheel forward
    digitalWrite(motorRB, LOW);
    Serial.println("Moving Left");
  }

  else if (newDir == 'R') { //forward
    digitalWrite(motorLF, HIGH);   //left wheel forward
    digitalWrite(motorLB, LOW);
    digitalWrite(motorRF, LOW);
    digitalWrite(motorRB, HIGH);  //right wheel backward
